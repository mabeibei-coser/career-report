...
该章节要在意向岗位的大类之下，给出 3 个细分岗位方向，并标明首选 / 次选 / 备选。
严格输出以下 JSON，不加额外文字：
...
要求：
1. subPositions 必须恰好 3 个，priority 分别是 primary、secondary、tertiary（顺序对应首选 / 次选 / 备选）
...
4. 细分岗位要和用户意向岗位强相关（例：意向"产品经理"可细分"B端产品""C端产品""AI 产品"；意向"数据分析师"可细分"业务分析师""产品数据分析师""BI/报表分析师"）
5. 每个能力描述 30-60 字，避免空泛
6. **fitReason 必须基于候选人的真实信息**（铁律，违反即失败）：
   - 首选 / 次选 / 备选每个 fitReason 必须引用**至少 1 条性格测评答案**的具体观察（例"测评显示你 J 倾向偏强"、"你在第 4 题选了偏结构化的选项"）
   - 如有简历，必须引用**至少 1 条简历证据**（例"你在 X 实习中做过需求梳理"）
   - 首选方向要讲清**为什么 TA 比另外两个方向更适合**
   - 备选方向要明确说出**TA 当前有哪些具体缺口**，并给一个入职前可补的动作
...
export async function POST(req: NextRequest) {
  try {
    ...
    const userPrompt = `${buildBaseContext(formData, quizAnswers)}\n\n请为"${formData.targetPosition}"这个意向岗位生成岗位信息 JSON（3 个细分岗位 + 每个 5 维能力）。`;
