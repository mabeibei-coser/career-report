...
| 编号 | 问题 / 复用场景 | 原因状态 |
|---|---|---|
| [A000-K001](#a000-k001) | 动态朗读无声、missing Authorization header、旧接口资源不匹配 | 协议缺陷已确认；已发布并验证真实合成 |
<!-- EXPERIENCE_INDEX_END -->

## 经验条目
<a id="a000-k001"></a>
### A000-K001｜语音接口协议与Uranus音色资源不匹配
...
- 处理与适用边界：基于版本2.4.2、HEAD 14a8a3f的工作区修复；切v3单向HTTP接口，X-Api-App-Id/Access-Key及seed-tts-2.0；逐段解码后拼MP3，必须有完成标志，出错仍返回空音频；保留本项目原重试行为，mock模式不外调。脚本与运行时复用项目内provider，无跨仓依赖。只适用于当前2.0音色；未来换1.0音色需匹配资源，不能机械套用。
- 防再犯检查：Node24执行 `node --test tests/volc-tts.test.mjs`；合并前运行项目build与相关测试；发布前使用准确发布构建验证真实合成和音频解码，不能只看HTTP200、mock或题目文字出现。
- 验证范围与日期：2026-09-22；语音回归7/7；生产构建通过；现有完整流程iPhone/Pixel各1项通过。 本轮未逐项目调用付费真实合成，未部署/重启生产，未重新生成静态音频；不能据此声称线上修复或真机扬声器验收。
...
- 同步移除旧scripts/test-volc-tts.mjs里的硬编码凭证并改为环境输入；历史Git提交未重写、凭证未轮换。
...
- `scripts/generate-tts-cache.mjs` SHA256 `9b8ae6eac9469212d0b35f109233649e249804dc29d8f4a386ea34a50b7a5444`
- `scripts/test-volc-tts.mjs` SHA256 `18b4e409795e71dbf6082e46904c3571a01ec1f26888701e5179f8d12bd206cd`
...
- 版本 v2.4.4，发布源码 94be5edb696047d17d7ef160adc4347bbad52b09；本地tag/GitHub tag/生产发布源码已核对。服务器准确源码语音契约及生产构建通过，隔离实例真实合成成功后才切换；生产接口返回MP3 133920字节/6.696秒、24000Hz，解码peak=0.3818。
...
- 回滚构建：/var/tmp/codex-cross-tts-release-20260922/A000/previous.next；旧源码 4ec76bd0059ba7a9730c6fd6dcdb977a009dad55。发布后新增错误日志0字节，其他PM2进程PID未改变。未改生产env/业务数据/静态音频；A000/A100额外验证正式源码实时合成。
- 证据：/var/tmp/codex-cross-tts-release-20260922/A000 下canary-check.json、production-check.json及build.log；本机Temp/codex-cross-tts-20260922/A000-browser.json。
