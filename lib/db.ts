...
// career-report 的 sqlite 句柄：只服务于职业定位的用户前台（reports 表）。
// admin 表 (admins / service_tracking / service_tracking_records) 已迁到 admin-hub —— 不要再加回这里。
// admin-hub 通过 env DB_PATH 指到同一个 .db 文件读写 admin 自己的表。
