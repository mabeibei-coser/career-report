...
// **已按 2024-2025 市场行情 × 0.9 下调**（用户反馈原值对大多数应届偏高约 10%）。
// 不要套用 top-tier 大厂 top-30% 录用数字，那是 outlier 不是 median。
export const salaryAnchors: SalaryAnchor[] = [
  {
    keywords: ["产品", "产品经理", "pm"],
    label: "产品类",
    tier1: { low: 5400, median: 7700, high: 12600 },
  },
  {
    keywords: ["研发", "开发", "后端", "前端", "全栈", "工程师", "软件", "算法", "ai", "机器学习", "数据科学"],
    label: "技术研发类",
    tier1: { low: 9000, median: 13500, high: 22500 },
  },
  {
    keywords: ["测试", "qa", "运维", "sre", "devops"],
    label: "测试运维类",
    tier1: { low: 7200, median: 9900, high: 14400 },
  },
  {
    keywords: ["数据分析", "bi", "分析师"],
    label: "数据分析类",
    tier1: { low: 6300, median: 9000, high: 13500 },
  },
  {
    keywords: ["设计", "ui", "ux", "视觉", "交互"],
    label: "设计类",
    tier1: { low: 5400, median: 7700, high: 11700 },
  },
  {
    keywords: ["运营", "内容", "市场", "市场营销", "marketing"],
    label: "运营/市场类",
    tier1: { low: 4500, median: 6300, high: 9900 },
  },
  {
    keywords: ["销售", "bd", "商务"],
    label: "销售/商务类",
    tier1: { low: 4500, median: 7200, high: 12600 },
  },
  {
    keywords: ["人力", "hr", "人事", "招聘", "薪酬"],
    label: "人力资源类",
    tier1: { low: 4500, median: 6300, high: 9000 },
  },
  {
    keywords: ["财务", "会计", "审计", "税务"],
    label: "财务类",
    tier1: { low: 4500, median: 5850, high: 9000 },
  },
  {
    keywords: ["法务", "律师", "合规"],
    label: "法务合规类",
  ...
  },
  {
    ...
    label: "咨询类",
    tier1: { low: 7200, median: 10800, high: 18000 },
  },
];
...
export function matchAnchor(position: string): SalaryAnchor {
  ...
  return matched ?? {
    keywords: [],
    label: "通用岗位",
    tier1: { low: 4500, median: 6750, high: 10800 },
  };
}
...
export function buildSalaryAnchorPrompt(
  ...
  const lines = [
    ...
    `**重要**：这些锚点已按 2024-2025 市场行情 ×0.9 下调对准真实中位（原行业数据对多数应届偏高约 10-15%）。**不要**再给互联网大厂 top-30% 录用的数字（如数据分析 15K+、产品 14K+），那是 outlier 不是 median。宁可略低于中位、不要高于高分位。`,
  ];
  ...
  if (education) {
    ...
    if (education.includes("博士")) {
      ...
      lines.push(`→ 本科（含 985/211 与普通本科混合池）：按锚点下调 5%（普通本科是主力市场，避免被 top-tier 拉高）。`);
